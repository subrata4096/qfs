...
int DistributedRepair::Run(DistributedRepairChunkOp* dRCOp)
{
      ...
      {
           PartialRepairOp = (dRCOp->opSequenceList).front();

           KFS_LOG_STREAM_ERROR << " subrata : PartialRepairOp string value = " << PartialRepairOp.stringValue << KFS_LOG_EOM;


           (dRCOp->opSequenceList).pop_front();
      }
}
